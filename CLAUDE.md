...
- **Textures peintes** : onze textures (sable, herbe, enduit, planches, chaume, basalte, bambou, rondins, écorce, tapa, feuillage) viennent d'une grille générée par Gemini (`textures/grille-gemini.webp`, 4 × 4 cases). `python3 outils/decouper-grille.py` la découpe en `textures/<nom>.webp` (256 × 256, bords raccordés, teinte moyenne ramenée sur celle de l'ancienne texture dessinée pour que la lumière reste juste) ; `node outils/integrer-textures.js` les **intègre dans `docs/fenua.html`** (bloc `<script type="application/json" id="textures-peintes">` entre deux repères), pour que le fichier reste autonome et marche même ouvert depuis le disque. Dans le jeu, `PEINTES` dit quelle texture remplace quelle `TEX.*`, `PEINTES_REPEAT` resserre les motifs trop gros, `PEINTES_MELEE` garde une part de l'ancienne par-dessus (l'enduit garde ses briques). Une texture absente ou illisible laisse la texture dessinée en place. Le sol se répète tous les 7 m et le shader du terrain module sa clarté avec la même texture lue dix fois plus grand, pour casser la répétition. Les deux cases de terre battue et les variantes de la ligne 4 (écorce, tapa, feuillage) ne servent pas encore.
