...
- **Cachettes** : six familles. `SOFT_HIDE` (nīʻau, abri, filets) amortit une chute ; `STAND_HIDE` (tonneaux, pirogue retournée) cache un homme debout, les autres exigent d'être accroupi. `HIDE_NOTE` et `HIDE_STASH` donnent les textes. **Un garde qui enquête fouille les deux cachettes les plus proches de son point d'enquête** (`hidesNear`, `fouillerCachette`) : il t'y trouve, et il y trouve aussi les corps dissimulés. Tant qu'il reste une cachette à fouiller, c'est elle qui mène ses pas — sinon le point d'enquête le rappellerait à chaque image.
